...
Then generate the second path by slightly modificated dijkstra with penalty for
each color visited by the first path. Then generate the third path with
penalties for colors on second path, and so on.
After 973ms return the last two paths generated.

 --- Details of choosing the first path ---
The first path is chosen from a few of candidates generated by various
algorithms: BFS, DFS, randomized BFS.
I generate a second path for each of the candidates and take the best pair to
the next stage.

 --- Details of dijksta algorithm ---
...
In order to do this, for each vertix all the colors visited on the way to this
vertix should be remembered. In order to save memory and processor time I use
persistent data structure for these color sets.
...
// So path [1 2 3 4 5] corresponds to vector {5, 4, 3, 2, 1}
typedef vector<int16_t> Path;

// I represent a query by start vertix, end vertix and best pair of paths
// currently found for them
...
^- here I have read about such a data structure called persistent vector.

My implementation of persistent vector of bools has these hardcoded properties:
...
but it looks like there were no such tests.
*/
...
// Every leaf stores 16 boolean values that represent 16 colors:
// if a set contains some color, the corresponding bit will be equal to one.
...
int set_insert(int root, int elem) {
	// Btw, if you ask, I can provide more readable, recursive code.
...
// The algo pops the vertex with minimum cost out of queue at each iteration.
// So I have implemented a comparator for that purpose only.
...
Path dijkstra(int16_t s, int16_t t, vector<bool> other_path_colors) {
	...
	// Delete all nodes of persistent vector before every run.
	bits = {0};
	set_nodes = {{}};
...
int main() {
	...
	// except the first one, which is too small.
	...
	for (int i = 0; i < q; i++) {
		...
		// Save the best candidate with its corresponding second path
	...
	for (int i = 0;; i++) {
		...
		// the number of iterations already performed on currently processed query
		int query_iteration = i / q;

		auto &[u, v, paths] = QUERIS[i % q];
		// In first itearation I improve the first path using the second one.
		// In next itearation I improve the second path using the first one.
		// In next itearation I improve the first path using the second one.
		// And so on.
		if (query_iteration % 2)
			paths.second = dijkstra(u, v, get_colors(paths.first));
		else
