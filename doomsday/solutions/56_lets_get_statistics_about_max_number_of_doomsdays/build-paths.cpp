...
public:
    inline bool operator() (const QueueEntry left, const QueueEntry right) { // TODO use references
...
inline Path greedy(int16_t s, int16_t t, vector<bool> risks) {
	...
	if (DEBUG)
		cout << s << "->" << t << "; cost=" << cost_of_the_path << endl;
	//if (cost_of_the_path / doomsday_cost > 64) *((int *) NULL) = 0;
	Path path;
	path.push_back(t);
	while (t_node != 0) {
