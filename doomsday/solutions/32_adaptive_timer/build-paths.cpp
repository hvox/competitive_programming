...
class queue_entity_comparator {
public:
    inline bool operator() (const tuple<int, int, int> left, const tuple<int, int, int> right) {
...
inline vector<int> dijkstra(int s, int t, vector<bool> risks) {
	...
	vector<int> visited(n + 1);
	priority_queue<tuple<int, int, int>, vector<tuple<int, int, int>>, queue_entity_comparator> Q;
	Q.push({0, s, -1});
...
int main() {
	...
	vector<int> delta_times(q);
	vector<vector<int>> empty_list(0);
	for (int i = 0; i < q; i++) {
		int t0 = CURRENT_TIME;
		int u, v; cin >> u >> v;
		queris[i] = {u, v, {simple_dfs(u, v), simple_bfs(u, v)}};
		delta_times[i] = (CURRENT_TIME - t0) * 10;
	}

	for (int i = 0;; i = (i + 1) % (2*q)) {
		if (CURRENT_TIME + delta_times[i % q] > 990666) break;
		int t0 = CURRENT_TIME;
		...
		delta_times[i % q] = CURRENT_TIME - t0;
	}
