	...
	while ((not Q.empty()) and !visited[t]) {
		...
		for (auto &[v, edge] : G[u]) {
			...
			for (int risk : edge_risks) {
				...
				if (array_contains(new_last_risks, risk)) continue;
				new_last_risks[(d2++) % cache_size] = risk;
...
typedef pair<uint16_t, int16_t> QueueSmallEntry;
struct queue_small_entity_comparator {
public:
    inline bool operator() (const QueueSmallEntry left, const QueueSmallEntry right) {
	return left.first > right.first;
    }
};

inline Path small_dijkstra(int16_t s, int16_t t) {
	int n = G.size() - 1;
	vector<bool> visited(n + 1);
	vector<int16_t> prevs(n + 1); prevs[s] = -1;
	vector<int> dists(n + 1); for (int i = 1; i <= n; i++) dists[i] = 0x7fffffff;
	priority_queue<QueueSmallEntry, vector<QueueSmallEntry>, queue_small_entity_comparator> Q;
	Q.push({0, s});
	while (!visited[t]) {
...
int main() {
	...
	for (int i = 0; i < q; i++) {
		...
		queris[i] = {u, v, {{}, small_dijkstra(u, v)}};
	}
	...
	for (int i = 0; ; i++) {
		...
		if ((i % (2*q)) < q) paths.first = dijkstra(u, v, get_doomsdays(paths.second));
