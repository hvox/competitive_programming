...
namespace greedy_with_randomness {
...
inline Path greedy(int16_t s, int16_t t) {
	...
	if (min_cost[t] == 0x7fffffff) return greedy(s, t);
	Path path = {t};
...
inline Path greedy(int16_t s, int16_t t, const vector<bool> risks) {
	vector<SetNode> set_nodes = {{0, 0, 0, 0}, {1, 1, 1, 1}};
	int n = G.size() - 1;
	vector<int> min_cost(n + 1, 0x7fffffff);
	vector<int16_t> parents(n + 1);
	...
	if (min_cost[t] == 0x7fffffff) return greedy(s, t, risks);
	Path path = {t};
	while (t != s) path.push_back(t = parents[t]);
	return path;
}
...
int main() {
	...
	for (int i = 0; i < q; i++) {
		...
		queris[i] = {u, v, {{}, greedy_with_randomness::greedy(u, v)}};
	}

	for (int i = 0; i < q || true; i++) {
		if (CURRENT_TIME > 983666) break;
		auto &[u, v, paths] = queris[i % q];
		if ((i % (2*q)) < q) paths.first = greedy::greedy(u, v, get_doomsdays(paths.second));
		else paths.second = greedy::greedy(u, v, get_doomsdays(paths.first));
	}
